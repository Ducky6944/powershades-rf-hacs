...
## What's built (as of 2026-09-18 — v0.1.9, local-first, cloud dropped)
...
**0.1.9 (this):** (1) **names now save AND apply** — user `CONF_CHANNEL_NAMES` override the gateway `chnames` in cover/device/sensor names (they were stored but never read before); (2) **Back / Forward** through the config flow works (every step idempotent + re-seeds captured values); (3) **per-shade position source** — `estimate` (default, time-based) vs `gateway` (live read), set in identify step + options flow; cover slider honors it and `set_position`/open/close update the estimate up front.
