...
## What's built (as of 2026-09-18 — v0.1.10, local-first, cloud dropped)
...
- **cover.py** — one cover per linked channel (open/close/stop + **SET_POSITION via `_movement.set_position`** with `from_position=estimate`, using `travel_time_for(channel)`) + one per user group (open/close/stop fan-out). All set `assumed_state=True`. `current_cover_position` honors `position_source_for(channel)` (estimate → our record; gateway → live read). Identity percent mapping; `available` requires the channel linked.
...
1. **Confirm in the real HA box (0.1.10):** HACS update → restart → re-run setup (identify loop: nudge until the shaded window matches, name it, set its travel time + position source). Verify: covers work, **travel-time number** updates the next move, **Reset button** re-syncs, **set to N%** lands near the requested % (shortest-path when known), group fan-out, 3 s poll feels responsive, and the logo shows in the HA UI. No `AttributeError` in the log.
