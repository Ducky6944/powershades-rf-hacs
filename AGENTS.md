...
## What's built (as of 2026-09-18 — v0.1.11, local-first, cloud dropped)
...
- **cover.py** — one cover per linked channel (open/close/stop + **SET_POSITION via `_movement.set_position`** with `from_position=estimate`, using `travel_time_for(channel)`) + one per user group (open/close/stop/**SET_POSITION** fan-out, parallel). All set `assumed_state=True`. Position via `coordinator.resolve_position(channel)` (estimates by default, opt-in to gateway live read); **groups** show the shared position when all members agree, else **unknown** + `position_mixed`/`position_min`/`position_max` extra attributes. Identity percent mapping; `available` requires the channel linked.
...
1. **Confirm in the real HA box (0.1.11):** HACS update → restart → re-run setup (identify loop: nudge until the shaded window matches, name it, set its travel time + position source). Verify: covers work, **travel-time number** updates the next move, **Reset button** re-syncs, **set to N%** lands near the requested % (shortest-path when known), **group set-to-N% fans out** (parallel) + mixed groups show **unknown** with `position_mixed`, 3 s poll feels responsive, and the logo shows in the HA UI. No `AttributeError` in the log.
